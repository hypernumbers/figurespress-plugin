...
sign2(PrivateKey, Str) ->
    io:format("Str is ~p~nPrivateKey is ~p~n", [Str, PrivateKey]),
    Sign = xmerl_ucs:to_utf8(Str),
    io:format("Sign is ~p~n", [Sign]),
    Hash = crypto:sha_mac(PrivateKey, Sign),
    io:format("Hash is ~p~n", [Hash]),
    Binary = binary_to_list(base64:encode(Hash)),
    io:format("Base64 is ~p~n", [base64:encode(Hash)]),
    io:format("List is ~p~n", [Binary]),
    Binary.
